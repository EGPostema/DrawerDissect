...
def extract_label_info(image_path, model, confidence, overlap, label_coordinates_dir):
    ...
    drawer_id = filename.split('_tray')[0]
    tray_id = filename.replace('_1000.jpg', '')

    label_info = {
        'tray_filename': filename,
        'barcode': '',
        'label': '',
        'geocode': '',
...
def transcribe_labels_and_ids(resized_trays_dir, api_key, model_endpoint, version, confidence, overlap):
    ...
    if os.path.exists(csv_path):
        ...
        existing_files = set(existing_df['tray_filename'])
        existing_transcriptions = set(existing_df.dropna(subset=['barcode', 'label', 'geocode'])['tray_filename'])
    else:
        existing_files = set()
