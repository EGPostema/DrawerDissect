    ...
    for root, _, files in os.walk(resized_dir):
        for f in files:
            if f.endswith('_1000.jpg'):
                # Store relative path from resized_dir for compatibility
                rel_path = os.path.relpath(os.path.join(root, f), resized_dir)
                resized_filenames.append(rel_path)
