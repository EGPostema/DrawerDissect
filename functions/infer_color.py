...
def infer_color(input_dir, output_dir, rf_instance, workspace_instance, model_endpoint, version, confidence=50, overlap=50):
    """
    Detect colorstandard in drawer images using the Roboflow model.
    ...
    for i, file_path in enumerate(image_files, 1):
        ...
        # Changed: Create filename with _color.json suffix
        base_name = filename.replace('.jpg', '')
        json_path = os.path.join(output_dir, f"{base_name}_color.json")
        
        # Check if JSON file already exists
        if os.path.exists(json_path):
            log_progress("find_color", i, total_files, f"Skipped (already exists)")
            skipped += 1
            continue
        ...
        try:
            ...
            log_progress("find_color", i, total_files, f"Found {tray_count} trays")
            processed += 1
