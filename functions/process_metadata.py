...
def extract_project_name(metadata_path: str) -> Optional[str]:
    ...
    try:
        ...
        if not project_name_match:
            print(f"Error: No project name found in {metadata_path}")
            return None
            
        # Convert spaces to underscores
        project_name = project_name_match.group(1)
        formatted_name = '_'.join(project_name.split())
        print(f"Extracted project name: {project_name} -> {formatted_name}")
        return formatted_name
        
    except Exception as e:
        print(f"Error: Failed to extract project name from {metadata_path} - {e}")
        return None
...
def process_files(metadata_dir: str, image_dir: str, output_file: str):
    ...
    if not os.path.exists(metadata_dir) or not os.path.exists(image_dir):
        raise FileNotFoundError("Metadata or image directory does not exist")
    ...
    if results:
        ...
        print(f"\nComplete. Processed: {len(results)}")
    else:
